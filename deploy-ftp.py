...
def sync(server: str, user: str, remote: str, dist: Path) -> int:
    ...
    try:
        ...
        # Only files this app actually ships are verified. CivTS has no `.htaccess` (it is a
        # single-page canvas game with no client-side routes to rewrite), so requiring one would
        # fail every deploy over a file the app was never supposed to have.
        shipped = [name for name in ("index.html", ".htaccess") if (dist / name).exists()]
        for critical in shipped:
