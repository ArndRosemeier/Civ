...
- **Integer-only simulation math** (food, shields, commerce, gold, beakers, culture). Authoritative *simulation* values are integers, so they cannot drift.
  *Precision of the rule (corrected in M1):* the guarantee is about arithmetic and portability, not about the serialised format. IEEE-754 `+ - * /` are exactly specified by ECMA-262 and number→string is fully specified, so a *configuration* float such as `settings.ai.aggression = 0.5` is deterministic and legal in state. What is banned is non-portable math — transcendentals (`Math.pow/sin/cos/log/exp/tan/…`), whose results come from each engine's libm — and float accumulation in simulation values. The lint bans express exactly that, and an adversarial review confirmed no float could be shown to cause a determinism defect.
...
- **Ownership rules (fixes rev 1's conflict risk):** only the main agent writes `PLAN.md`, `TASKS.md`, `GDD.md`, `INTERFACES.md`. Subagents return results or write to **disjoint files** (one file per writer). No two writers on one file.
- **Delegation-first (rev 3).** The principal's standing instruction: *the chief of staff orchestrates agents and rarely writes implementation code.* The main agent's work is: freeze interfaces (`docs/INTERFACES.md`), set acceptance criteria, launch and steer agents, adjudicate contract disputes, run the verification gate, commit, and report. Implementation, tests, and docs are written by subagents.
  - Hand-coding is reserved for: bootstrapping (the M0 toolchain/lint/gate that agents then work inside), and unblocking integration failures that no agent can resolve. Both are exceptional and must be justified in the ledger.
  - Delegation is safe here *because* of the gate: strict types catch interface drift mechanically, and `pnpm verify` (≈0.3 s) plus goldens/invariants catch behavioural drift. The compiler — not the chief of staff — is the integration mechanism.
  - Parallelism is bounded by genuine dependency: `gen` → `state` → `textview`/goldens. Do not fake concurrency across a real dependency; sequence it and say so.
  - Every workstream prompt must be self-contained (agents share no conversation context) and must name: exact files, exact signatures from `docs/INTERFACES.md`, the determinism constraints, and the requirement to run `pnpm typecheck` plus its own tests before reporting.
  - **Author and verifier are different agents.** Whoever writes a module does not get to certify it; a separate adversarial reviewer tries to falsify determinism, goldens, and value ranges.
